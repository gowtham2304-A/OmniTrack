...
from ..models import Platform, DailyPlatformMetric
...
@router.get("/")
def list_platforms(db: DbDep) -> list[PlatformOut]:
    return db.query(Platform).order_by(Platform.name).all()


@router.get("/active")
def list_active_platforms(db: DbDep) -> list[PlatformOut]:
    return db.query(Platform).filter(Platform.is_active == True).order_by(Platform.name).all()


@router.get("/summaries")
def get_platform_summaries(db: DbDep) -> list[PlatformSummary]:
    today = date.today()
...
def create_platform(platform: PlatformCreate, db: DbDep) -> PlatformOut:
    p = Platform(**platform.model_dump())
    db.add(p)
    db.commit()
    db.refresh(p)
    return p
...
def get_platform(platform_id: int, db: DbDep) -> PlatformOut:
